...
const isSchemaWorkspace = async () => {
  ...
  if (foundPackageFile) {
    ...
    if (collectionFilePath) {
      const collectionFilePhysicalPath = getPhysicalFilePath(collectionFilePath);
...
const getCollectionFilePath = () => {
  const packageJsonFilePath = getPackageJsonFilePath();
  const packageJsonContent = JSON.parse(fs.readFileSync(packageJsonFilePath).toString('UTF-8'));
